...
def get_nganh():
    ...
    message = "Xin lỗi, tôi không thể tìm kiếm thông tin về ngành đã cho."
    return jsonify({"payload": message}), 400
# Lấy all ngành
...
# TEST VIẾT LẠI TRƯỜNG HỢP NHẬP KHÔNG ĐÚNG TÊN NGÀNH
# @api_bp.route('/lay-nganh', methods=['GET'])
# @csrf.exempt
# def get_nganh():
...
#         ten_nganh = match.group(1).strip().lower()
#         print(ten_nganh)
#         if ten_nganh: #ten nganh khac rỗng
#             tuyen_sinh = db.session.query(
#                 TuyenSinh).filter_by(ten_nganh=ten_nganh).first()
#             print(tuyen_sinh)
#             if tuyen_sinh:
...
#                 return jsonify({"payload": message, "ten_nganh": tuyen_sinh.ten_nganh}), 200
#             else:
#                 ten_ma_nganh = tuyen_sinh = db.session.query(
#                 TuyenSinh).first()
#                 print(ten_ma_nganh)
#                 message = f"Tên ngành {ten_nganh} không chính xác."
#                 message += f"\n- Danh sách ngành và mã ngành: {ten_ma_nganh}"
#                 return jsonify({"payload": message}), 400
#     print(message)
#     # message = "Xin lỗi, tôi không thể tìm kiếm thông tin với các thông tin đã cho."
#     message = "Xin lỗi, tôi không thể tìm kiếm thông tin về ngành đã cho."
